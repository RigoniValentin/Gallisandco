    ...
    <div className={`relative ${reversed ? "lg:order-2" : "lg:order-1"}`}>
      <div className="relative overflow-hidden rounded-[2rem] border border-accent/35 bg-white p-3 shadow-[0_34px_90px_-48px_rgba(35,51,73,0.42),0_0_0_1px_rgba(255,255,255,0.7)] ring-1 ring-white/70 backdrop-blur-sm">
        <div className="relative aspect-[4/5] overflow-hidden rounded-[1.55rem] bg-[#f8f3ea]">
          ...
          <div
            aria-hidden
            className="absolute inset-0 bg-[linear-gradient(180deg,rgba(255,255,255,0.02)_0%,rgba(242,211,154,0.12)_48%,rgba(35,51,73,0.16)_100%)]"
          />
  ...
  return (
    <div className={`flex items-center ${reversed ? "lg:order-1" : "lg:order-2"}`}>
      <div className="max-w-2xl py-2 md:py-4 lg:pl-2">
        <span className="inline-flex items-center gap-3 text-eyebrow font-semibold uppercase text-[#8b623f]">
        ...
        <p className="mt-6 text-[11px] uppercase tracking-wider2 text-[#2a405f]">{sister.summary}</p>
      </div>
    </div>
  );
}
...
function SisterProfileRow({
  ...
  return (
    ...
    >
      ...
      <SisterDescriptionCard sister={sister} reversed={reversed} />
    </motion.article>
  );
}
    ...
    >
      <div className="relative aspect-[4/5] overflow-hidden bg-[#f8f3ea]">
        <img
          ...
          loading="lazy"
        />
        <div className="absolute inset-0 bg-[linear-gradient(135deg,rgba(255,255,255,0.08),rgba(242,211,154,0.1)),radial-gradient(circle_at_18%_20%,rgba(242,211,154,0.18),transparent_30%),radial-gradient(circle_at_85%_80%,rgba(169,122,81,0.14),transparent_34%)]" />
        <div className="absolute inset-x-4 top-4 flex items-center justify-between gap-3">
      ...
      <div className="border-t border-accent/20 bg-[linear-gradient(180deg,rgba(255,255,255,0.99),rgba(242,211,154,0.24))] px-6 pb-6 pt-5 backdrop-blur-2xl">
        ...
        <ul className="mt-4 space-y-2 text-[14px] leading-relaxed text-[#0a1a33] md:text-[15px]">
          {member.bioItems.map((item) => (
            <li key={`${member.name}-${item}`} className="flex gap-3">
...
function NetworkSlideshow() {
  ...
  useEffect(() => {
    const id = setInterval(() => {
      setIndex((currentIndex) => (currentIndex + 1) % networkCountries.length);
    }, 4200);
  ...
  return (
    <>
      ...
      <div className="absolute bottom-5 right-5 flex items-center gap-1.5">
        {networkCountries.map((country, itemIndex) => (
          <span
            key={country.code}
            aria-hidden
            className={`h-1.5 rounded-full transition-all duration-500 ${
              itemIndex === index ? "w-5 bg-accent" : "w-1.5 bg-white/40"
            }`}
          />
        ))}
      </div>
    ...
    >
      ...
      <div className="container relative z-10">
        ...
        >
          ...
          <div className="mx-auto mt-6 space-y-10 lg:max-w-5xl lg:space-y-14">
        ...
        >
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-3">
            {ofCounsel.map((member, index) => (
              <OfCounselCard key={member.name} member={member} index={index} />
            ))}
          </div>
        </motion.div>
            ...
            <div className="relative min-h-[240px] overflow-hidden lg:min-h-[340px]">
              ...
              <div className="pointer-events-none absolute inset-x-5 bottom-5 flex items-end">
                <div className="max-w-[12rem] rounded-[1.1rem] border border-white/70 bg-[linear-gradient(180deg,rgba(255,255,255,0.96),rgba(245,242,240,0.88))] p-3 shadow-[0_16px_36px_-26px_rgba(35,51,73,0.42)] backdrop-blur-2xl">
                  ...
                  <p className="mt-1 text-sm leading-relaxed text-[#0a1a33]">{t("globalReachText")}</p>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col justify-center p-7 md:p-10">
              ...
              <div className="mt-7 grid grid-cols-2 gap-2 sm:grid-cols-4">
                ...
                <motion.span
                  ...
                  className="relative col-span-2 flex items-center justify-center overflow-hidden rounded-full border border-accent/25 bg-[linear-gradient(135deg,rgba(255,255,255,0.98),rgba(242,211,154,0.22))] px-3 py-2 text-[11px] font-medium whitespace-nowrap tracking-wider2 text-[#8b623f] backdrop-blur sm:col-span-4"
                >
                  <motion.span
                    aria-hidden
              ...
              <div className="mt-7 flex flex-wrap items-center gap-2 text-[11px] uppercase tracking-wider2 text-[#8b623f]/80">
                <span className="inline-flex items-center gap-1.5 rounded-pill border border-accent/20 bg-[linear-gradient(135deg,rgba(255,255,255,0.98),rgba(242,211,154,0.14))] px-3 py-1.5 shadow-[0_10px_24px_-18px_rgba(35,51,73,0.18)] backdrop-blur">
                  <Languages className="h-3.5 w-3.5 text-[#8b623f]" /> {languages.join(" · ")}
                </span>
              </div>
            </div>
          </div>
        ...
        >
          ...
          <div className="relative flex flex-col gap-6 p-7 md:p-10 lg:flex-row lg:items-center lg:justify-between">
            <div className="max-w-3xl">
              <span className="inline-flex items-center gap-3 text-eyebrow uppercase text-[#d7b37b]">
                <span className="h-px w-10 bg-accent/70" />
                {t("conversationCta")}
              </span>
              <p className="mt-4 text-base leading-relaxed text-white/84 md:text-lg">
                {closingCopy}
              </p>
            </div>

            <Link href="/contact" className="gold-cta group inline-flex items-center gap-2 px-7 py-3.5 text-sm font-semibold">
              {t("conversationCta")}
